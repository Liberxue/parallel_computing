...
__device__ int findMSB(unsigned int num) {
    if (num == 0) return -1;
    int msb = 0;
    while (num >>= 1) {
        msb++;
...
__global__ void findMSBKernel(unsigned int* input, int* output, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        output[idx] = findMSB(input[idx]);
    }
}
...
int main() {
    ...
    findMSBKernel<<<blocksPerGrid, threadsPerBlock>>>(dev_input, dev_output, n);

    int* output = new int[n];
    cudaMemcpy(output, dev_output, n * sizeof(int), cudaMemcpyDeviceToHost);

    for (int i = 0; i < n; i++) {
        std::cout << "Input: 0x" << std::hex << input[i] << ", MSB position: " << output[i] << std::endl;
    }
